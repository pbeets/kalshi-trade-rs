...
//! nested markets hierarchy, and event metadata.
//!
//! Run with: cargo run --example events

use kalshi_trade_rs::{
    EventStatus, GetEventParams, GetEventsParams, KalshiClient, KalshiConfig,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(first_event) = open_events.events.first() {
        ...
        if let Some(markets) = &event_with_markets.event.markets {
            ...
            for market in markets {
                ...
                println!("  {} | {} | vol: {} | oi: {}", market.ticker, status, volume, oi);
    ...
    if let Some(first_event) = open_events.events.first() {
        ...
        println!("Found {} events in this series:", series_events.events.len());
